...
int32_t loopback_udpc(uint8_t sn, uint8_t *buf, uint8_t *destip, uint16_t destport)
{
    ...
    case SOCK_UDP:
        ...
        if ((size = getSn_RX_RSR(sn)) > 0)
        {
            if (size > DATA_BUF_SIZE)
                size = DATA_BUF_SIZE;
            ret      = recvfrom(sn, buf, size, dest_ip, (uint16_t *)&dest_port);
            buf[ret] = 0x00;
            if (destip[0] == dest_ip[0] && destip[1] == dest_ip[1] && destip[2] == dest_ip[2] && destip[3] == dest_ip[3] && dest_port == destport)
            {
                printf("%d: recv form[%d.%d.%d.%d][%d]: %s\r\n", sn, destip[0], destip[1], destip[2], destip[3], destport, buf);
                if (ret <= 0)
                {
