...
uint8_t CAN_Mode_Init(uint8_t tsjw, uint8_t tbs2, uint8_t tbs1, uint16_t brp, uint8_t mode)
{
    ...
    GPIO_Init(GPIOB, &GPIO_InitStructure);           //initialization IO
    ...
    CAN_InitStructure.CAN_AWUM = DISABLE; //sleep mode is awakened through software(eliminateCAN->sleep bit of mcr)
    ...
    //set baud rate
    CAN_InitStructure.CAN_SJW       = tsjw;                                   //resynchronize the jump width tsjw to tsjw 1 time unit  CAN_SJW_1tq	 CAN_SJW_2tq CAN_SJW_3tq CAN_SJW_4tq
    ...
    CAN_InitStructure.CAN_Prescaler = brp;                                    //division factor(Fdiv)by brp+1
...
//Other, the length of the received data.
